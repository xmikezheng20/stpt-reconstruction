...
stpt.illumination.stitchitReference.writeQC( ...
    audit, datasetIndex, cfg, model, stageDir);
...
fprintf(fid, "Fitted channel/layer pairs: %d/%d\n", ...
    nnz(audit.templateSummary.correctionApplied), ...
    height(audit.templateSummary));
fallbacks = audit.templateSummary(~audit.templateSummary.correctionApplied, :);
