    ...
    for layer = correctedLayers
        ...
        if isempty(gain)
            ...
            text(0.5, 0.5, "Identity fallback" + newline + ...
                zDiagnostic.correctionReason(layer), ...
...
fprintf(fid, "XY identity-processed comparison planes: %d\n", ...
    nnz(~manifest.xyIlluminationApplied));
fprintf(fid, "Z identity-fallback comparison planes: %d\n", ...
    nnz(~manifest.zIlluminationApplied & ...
    ~ismember(manifest.zIlluminationReason, ...
