...
%WRITEPILOTQC Write compact diagnostics for the final center-section output.
%
% QC reads canonical final TIFFs and, for one checkerboard, the same native
% tiles through the public correction interface. It never changes output data.
...
sgtitle(sprintf("Section %d: canonical %s fusion; z illumination %s", ...
    sectionNumber, fusionModeLabel(cfg), cfg.zIllumination.method));
...
imshow(overlay);
title(sprintf("Section %d, layer 1: red ch1 / green ch2 (%s; z %s)", ...
    sectionNumber, fusionModeLabel(cfg), cfg.zIllumination.method));
exportgraphics(fig, outputPath, "Resolution", 160);
close(fig);
end
...
for row = 1:height(geometry.placements)
    ...
    [~, isPresent] = stpt.io.resolveTileFile(datasetIndex, sectionNumber, ...
        layer, placement.acquisitionIndex, referenceChannel);
    if ~isPresent
        continue
    end
...
fprintf(fid, "Z illumination: %s; reference layer %d\n", ...
    string(cfg.zIllumination.method), cfg.zIllumination.referenceLayer);
fprintf(fid, "Z estimation: at most %.0f pixels; filter-area fraction %.6g\n", ...
...
fprintf(fid, "Planes with missing tiles: %d\n", ...
    nnz(manifest.missingTileCount > 0));
fprintf(fid, "Missing tile observations: %.0f\n", ...
    sum(manifest.missingTileCount, "omitnan"));
fprintf(fid, "Unsupported output pixels: %.0f\n", ...
...
fprintf(fid, "Planes using XY identity processing: %d\n", ...
    nnz(~manifest.xyIlluminationApplied));
