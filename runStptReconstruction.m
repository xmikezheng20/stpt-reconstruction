...
summary = audit.templateSummary;
fprintf("Stage 2 complete: %s used %d fitted and %d identity " + ...
    "channel/layer model(s) from %d training sections.\n", ...
    cfg.illumination.method, nnz(summary.correctionApplied), ...
    nnz(~summary.correctionApplied), ...
    numel(cfg.illumination.trainingSections));
fprintf("Outputs: %s\n\n", stageDir);
end
...
stpt.reconstruction.writePilotQC( ...
    datasetIndex, model, cfg, sectionNumber, manifest, stageDir);

% Optional comparisons are independent products. They use the same section
...
if cfg.qc.comparisons.reconstructionSteps
    comparisonManifest = ...
        stpt.reconstruction.writeReconstructionStepComparison( ...
